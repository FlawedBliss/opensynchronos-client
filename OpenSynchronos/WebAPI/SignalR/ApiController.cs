...
using OpenSynchronos.API.Dto;
using OpenSynchronos.API.Dto.User;
using OpenSynchronos.API.SignalR;
using OpenSynchronos.MareConfiguration;
using OpenSynchronos.MareConfiguration.Models;
using OpenSynchronos.PlayerData.Pairs;
using OpenSynchronos.Services;
using OpenSynchronos.Services.Mediator;
using OpenSynchronos.Services.ServerConfiguration;
using OpenSynchronos.WebAPI.SignalR;
using OpenSynchronos.WebAPI.SignalR.Utils;
using System.Reflection;

namespace OpenSynchronos.WebAPI;

#pragma warning disable MA0040
public sealed partial class ApiController : DisposableMediatorSubscriberBase, IOpenSynchronosHubClient
{
    public const string MainServer = "XIV";
    ...
    public async Task CreateConnectionsAsync()
    {
        ...
        if (!_serverManager.CurrentServer.UseOAuth2)
        {
            ...
            if (multi)
            {
                ...
                _connectionDto = null;
                Mediator.Publish(new NotificationMessage("Multiple Identical Characters detected", "Your Service configuration has multiple characters with the same name and world set up. Delete the duplicates in the character management to be able to connect to OpenSynchronos.",
        ...
        while (ServerState is not ServerState.Connected && !token.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                if (_connectionDto.ServerVersion != IOpenSynchronosHub.ApiVersion)
                {
                    if (_connectionDto.CurrentClientVersion > currentClientVer)
                    {
                        Mediator.Publish(new NotificationMessage("Client incompatible",
                            ...
                            $"This client version is incompatible and will not be able to connect. Please update your OpenSynchronos client.",
                            NotificationType.Error));
                    }
                ...
                if (_connectionDto.CurrentClientVersion > currentClientVer)
                {
                    Mediator.Publish(new NotificationMessage("Client outdated",
                        $"Your client is outdated ({currentClientVer.Major}.{currentClientVer.Minor}.{currentClientVer.Build}), current is: " +
                        $"{_connectionDto.CurrentClientVersion.Major}.{_connectionDto.CurrentClientVersion.Minor}.{_connectionDto.CurrentClientVersion.Build}. " +
                        $"Please keep your OpenSynchronos client up-to-date.",
                ...
                if (_dalamudUtil.HasModifiedGameFiles)
                {
                    ...
                    if (!_mareConfigService.Current.DebugStopWhining)
                        Mediator.Publish(new NotificationMessage("Modified Game Files detected",
                            ...
                            "OpenSynchronos, Penumbra, and some other plugins assume your FFXIV installation is unmodified in order to work. " +
                ...
                if (_dalamudUtil.IsLodEnabled && !_naggedAboutLod)
                {
                    ...
                    if (!_mareConfigService.Current.DebugStopWhining)
                    {
                        Mediator.Publish(new NotificationMessage("Model LOD is enabled",
                            ...
                            "random crashes when loading in or rendering modded pairs. Disabling LOD has a very low performance impact. Disable LOD while using OpenSynchronos: " +
